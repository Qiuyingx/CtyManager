...
function uploadFile(id) {
	$.ajaxFileUpload({
		url : path + "/content/uploadfilesss",
		secureuri : false,
		fileElementId : "fileUpload",
		dataType : "json",
		success : function(data, status) {
			if (data.isSuccess) {
				$("#uploadMsg").html("【" + data.srcFileName + "】上传成功");
				$("#"+id).val(data.fileUrl);
			} else {
				$("#"+id).val("");
				$("#uploadMsg").html("上传失败");
...
/**
 * 上传预览图
 * @param id 接收上传图片Url 隐藏元素ID
 * @param viewId 图片预览ID（img)
 */
function uploadViewImg(id,viewId,fileName, isPicShow, isFunction) {
	...
	$.ajaxFileUpload({
		...
		success : function(data, status) {
			$("#main").unmask();
			if (data.isSuccess) {
/*				if($("#"+id).attr("class").indexOf("easyui-textbox") >= 0) {
					$("#"+id).textbox("setValue", data.fileUrl);
				}else{
					$("#"+id).val(data.fileUrl);
				}*/
				$("#"+viewId).attr("src","../../"+data.fileUrl);
				if(isPicShow.length > 0) {
					$("#"+isPicShow).show();
				}
				//是否需要回调处理 true
				if(isFunction.length > 0 && isFunction == "true") {
					imageProcesser(data.fileUrl, isPicShow);
				}
			} else {
				$("#"+id).val("");
				$("#uploadMsg").html("上传失败");
