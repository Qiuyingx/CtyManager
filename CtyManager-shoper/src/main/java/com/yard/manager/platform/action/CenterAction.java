...
public class CenterAction extends JsonAction implements SessionAware, ServletRequestAware, ServletResponseAware {
	...
	private TrainInfoService tcs = TrainInfoService.getInstance();
	...
	public String execute() throws Exception {
		...
		// 将用户信息存入session中
		session.put(ManagerConstant.SESSION_USER, user);
		// 将用户培训室ID存入session中
		session.put(ManagerConstant.SESSION_TRAIN_ID, tcs.findById().getId());
		// 获取当前登录用户
