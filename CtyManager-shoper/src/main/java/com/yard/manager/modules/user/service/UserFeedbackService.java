...
public class UserFeedbackService extends BaseService<UserFeedbackViewEntity>{
	 ...
	 */
	public List<UserFeedbackViewEntity> queryUserFdList(UserFdQueryEntity query, long page, long rows, String courtyardIds) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT c.nickname nickName, c.tel, b.courtyard_name courtyardName, a.user_id userId, a.courtyard_id courtyardId, ");
		...
		sql.append("t_courtyard b, t_user c WHERE a.courtyard_id = b.id AND a.user_id = c.id ");
	 ...
	 * @return
	 */
	public long queryUserFdCount(UserFdQueryEntity query, String courtyardIds) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT count(*) FROM t_user_feedback a, t_courtyard b, t_user c WHERE a.courtyard_id = b.id AND a.user_id = c.id ");
	...
	public void queryParams(UserFdQueryEntity query, StringBuffer sql, List<Object> params) {
		if(query != null) {
			...
			if(!StringUtils.isBlank(query.getCourtyardIds())) {
				sql.append("AND b.id IN (" + query.getCourtyardIds() + ") ");
			}
			// 用户电话
