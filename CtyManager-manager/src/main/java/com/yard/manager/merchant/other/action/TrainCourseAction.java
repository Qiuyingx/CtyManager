...
import com.yard.manager.merchant.other.service.TrainCourseService;
 ...
 */
@Results({
	@Result(type = "freemarker", name = "traincourseindex", location = "/WEB-INF/content/merchant/other/traincourseindex.html"),
	@Result(type = "freemarker", name = "content_edit", location="/WEB-INF/content/merchant/other/traincourse_edit.html")
})
public class TrainCourseAction extends BaseAction {
	private static final long serialVersionUID = -3719531580009240976L;
	private static final String NAMESPACE = "/other/train/traincourse";
	...
	public String queryList() {
		try {
			map.put("total", tcs.queryCount(query, trainId));
			map.put("rows", tcs.queryList(page, rows, query, trainId));
		} catch (Exception e) {
			e.printStackTrace();
			map.put("total", 0);
